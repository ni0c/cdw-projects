...
function shortenHash(_hash) {
  let hash = String(_hash);
  return `${hash.slice(0, 8)}...${hash.slice(-6)}`;
}

function makeHashLink(hash) {
  let url = THIS_ENV.txurl + hash;
...
function showHashPopup(hash) {
  let link = makeHashLink(hash);
  showNotice("🔄 Transaction submitted. Hash: " + link);
}

function showHashReadyPopup(hash) {
  let link = makeHashLink(hash);
  showNotice("✅ Transaction confirmed. Hash: " + link);
}

function showHashErrorPopup(msg, hash) {
  let link = makeHashLink(hash);
...
async function animPause() {
	var _el = document.getElementById('videoEl');
  $('#videoEl').removeClass("bg-blur").addClass("bg-blur");
  $('#bg-img-overlay').removeClass("bg-darken").addClass("bg-darken");
  if (_el.paused) {
    return;
  }
  _el.pause();
...
{
	var _el = document.getElementById('videoEl');
  $('#videoEl').removeClass("bg-blur");
  $('#bg-img-overlay').removeClass("bg-darken");
  if (!_el.paused) {
    return;
  }
